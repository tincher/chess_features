...
class AbstractFeature(ABC):
    ...
    def __init__(self, board, is_midgame=None, color=chess.WHITE):
        self.board = board
...
class ExtractNonPawnMaterial(AbstractFeature):
    def __init__(self, board, is_midgame=None, color=None):
        super().__init__(board, is_midgame, color)

        self.values = [0, 781, 825, 1276, 2538] if self.is_midgame else [0, 854, 915, 1380, 2682]

    def extract_feature(self):
        values = dict(zip(["pawn", "knight", "bishop", "rook", "queen", "king"], self.values + [0]))
...
class ExtractPieceValue(AbstractFeature):
    ...
    def extract_feature(self):
        values = dict(zip(["pawn", "knight", "bishop", "rook", "queen", "king"], self.piece_values + [0]))
        return np.sum(get_unified_valued_bitboard(self.board, values=values))
...
class ExtractPsqt(AbstractFeature):
    def __init__(self, board, is_midgame=None, color=None):
        ...
        if self.is_midgame:
            ...
            self.piece_bonus = np.array([[[-96, -65, -49, -21], [-67, -54, -18, 8], [-40, -27, -8, 29],
                                          [-35, -2, 13, 28], [-45, -16, 9, 39], [-51, -44, -16, 17],
                                          [-69, -50, -51, 12], [-100, -88, -56, -17]],
                                         [[-57, -30, -37, -12], [-37, -13, -17, 1], [-16, -1, -2, 10], [-20, -6, 0, 17],
                                             [-17, -1, -14, 15], [-30, 6, 4, 6], [-31, -20, -1, 1],
                                             [-46, -42, -37, -24]],
                                         [[-9, -13, -10, -9], [-12, -9, -1, -2], [6, -8, -2, -6], [-6, 1, -9, 7],
                                          [-5, 8, 7, -6], [6, 1, -7, 10], [4, 5, 20, -5], [18, 0, 19, 13]],
                                         [[-69, -57, -47, -26], [-55, -31, -22, -4], [-39, -18, -9, 3],
                                          [-23, -3, 13, 24], [-29, -6, 9, 21], [-38, -18, -12, 1], [-50, -27, -24, -8],
                                          [-75, -52, -43, -36]],
                                         [[1, 45, 85, 76], [53, 100, 133, 135], [88, 130, 169, 175],
                                          [103, 156, 172, 172], [96, 166, 199, 199], [92, 172, 184, 191],
                                          [47, 121, 116, 131], [11, 59, 73, 78]]])
            self.pawn_bonus = np.array([[0, 0, 0, 0, 0, 0, 0, 0], [-10, -6, 10, 0, 14, 7, -5, -19],
                                        [-10, -10, -10, 4, 4, 3, -6, -4], [6, -2, -8, -4, -13, -12, -10, -9],
                                        [10, 5, 4, -5, -5, -5, 14, 9], [28, 20, 21, 28, 30, 7, 6, 13],
...
class ExtractMobilityArea(AbstractFeature):
    def extract_feature(self):
        own_mobility_area = self.get_mobility_area(self.board, self.color)
        opponent_mobility_area = self.get_mobility_area(self.board.mirror(), self.color)
        return own_mobility_area.sum() - opponent_mobility_area.sum()

    @classmethod
    def get_mobility_area(self, board, color):
        values = dict(zip(["pawn", "knight", "bishop", "rook", "queen", "king"], range(1, 7)))
        ...
        pins = self.get_pinned(board, color)
        for pin in pins:
            index, _ = pin
    ...
    @classmethod
    def get_pinned(self, board, color):
...
class ExtractMobility(AbstractFeature):
    def __init__(self, board, is_midgame=None, color=None):
        ...
        if self.is_midgame:
            self.bonus = [[-62, -53, -12, -4, 3, 13, 22, 28, 33],
            ...
            self.bonus = [[-81, -56, -31, -16, 5, 11, 17, 20, 25],
                          [-59, -23, -3, 13, 24, 42, 54, 57, 65, 73, 78, 86, 88, 97],
                          [-78, -17, 23, 39, 70, 99, 103, 121, 134, 139, 158, 164, 168, 169, 172],
                          [-48, -30, -7, 19, 40, 55, 59, 75, 78, 96, 96, 100, 121, 127, 131, 133, 136, 141, 147, 150,
                           151, 168, 168, 171, 182, 182, 192, 219]]

    def extract_feature(self):
        mobility_area = ExtractMobilityArea.get_mobility_area(self.board, self.color)[::-1].reshape(-1)
        mob = []
        for square in range(64):
            if self.board.color_at(square) == chess.WHITE:
                if self.board.piece_type_at(square) not in [None, 1,  6]:
...
class ExtractPawnlessFlank(AbstractFeature):
    def extract_feature(self):
        ...
        int
            who is in favor
        """
        return self.pawnless_flank_colored(chess.BLACK) - self.pawnless_flank_colored(chess.WHITE)

    def pawnless_flank_colored(self, color):
...
class ExtractStrengthSquare(AbstractFeature):
    def extract_feature(self):
        ...
        return self.get_strength_square(self.board, self.color) - self.get_strength_square(flipped_board,
                                                                                           not self.color)

    def get_strength_square(self, board, color):
        debug = np.zeros((8, 8))
        ...
        for square_x, square_y in itertools.product(range(8), repeat=2):
            ...
            for x_ in range(kx - 1, kx + 2):
                ...
                for y in range(0, square_y+1):
                    if ((board.piece_type_at((x_) + y * 8) == 1
                         and board.color_at((x_) + y * 8) == (not color))
                            and not (board.piece_type_at((x_ - 1) + (y-1)*8) == 1
