...
class TestChessNeighborhoods:
    ...
    def test_get_file_rank_neighbor(self):
        assert (
            get_file_rank_neighbor(
                self.board, x=2, y=1, down_right_of_square=1, file_wise=True
            )
            == 0
        )
        assert (
            get_file_rank_neighbor(
                self.board, x=2, y=1, down_right_of_square=1, file_wise=False
            )
            == -1
        )
        assert (
